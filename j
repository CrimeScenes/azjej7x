...
local function isWithinViewEra(position)
    ...
    local fovHeight = TriggerBotSettings.ViewEra.Vertical * 100
    local fovWidth = TriggerBotSettings.ViewEra.Horizontal * 100
...
local function isIgnoringKnife()
    ...
    if currentTool then
        ...
        return TriggerBotSettings.Safety.IgnoreKnife and (toolName == "knife" or toolName == "katana" or toolName == "[knife]" or toolName == "[katana]")
    end
    return false
end
...
local function getPredictedPosition(targetPlayer)
    ...
    if targetCharacter then
        ...
        if humanoid and humanoid.RootPart then
            ...
            local predictedPos = targetCharacter.PrimaryPart.Position + velocity * TriggerBotSettings.Preds.PredictionMultiplier
            return predictedPos
        end
    end
    return nil
        ...
        if humanoid and humanoid.Health > 0 and not isPlayerKnocked(TargetPlayer) then
            if isMouseOnTarget(TargetPlayer) then
                ...
                for _, part in pairs(bodyParts) do
                    local targetPosition = TriggerBotSettings.Preds.EnableLegitMode and getPredictedPosition(TargetPlayer) or part.Position
                    local screenPos, onScreen = Camera:WorldToViewportPoint(targetPosition)
                    if onScreen and isWithinViewEra(targetPosition) then
