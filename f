...
-- Helper function for prediction and hitbox adjustments based on distance
            ...
            if isMouseOnTarget(TargetPlayer) then
                ...
                for _, part in pairs(bodyParts) do
                    ...
                    if onScreen and isWithinViewEra(predictedPos) then
                        ...
                        -- New check: Only shoot if the mouse is within a certain distance of the target
                        local mouseX, mouseY = getMousePosition()
                        local distanceToTarget = (Vector2.new(mouseX, mouseY) - screenPos).Magnitude
                        local shootThreshold = 5  -- Distance threshold before shooting

                        -- Shoot only if close enough to the target
                        if distanceToTarget < shootThreshold and os.clock() - lastClickTime >= shared.Forbidden.TriggerBot.Settings.ResetTime then  
