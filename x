...
local lastPositions = {}
local basePredictionFactor = 0.15  -- Default base prediction factor
...
local function predictTargetPosition(targetPlayer, deltaTime)
    ...
    if humanoidRootPart then
        ...
        -- Dynamically adjust prediction factor
        local predictionFactor = basePredictionFactor
        if distance < 50 then
            predictionFactor = 0.05  -- Fine-tuned for close range
        elseif distance < 200 then
            predictionFactor = 0.15  -- Fine-tuned for mid range
        else
            predictionFactor = 0.25  -- Fine-tuned for long range
        end

        -- Predict future position
        local prediction = humanoidRootPart.Position + velocity * deltaTime * predictionFactor

        -- Smooth predictions using an average of recent positions
    ...
    if TargetPlayer and TargetPlayer.Character then
        ...
        if humanoid and humanoid.Health > 0 and not isPlayerKnocked(TargetPlayer) then
            ...
            for _, part in pairs(bodyParts) do
                local predictedPos = predictTargetPosition(TargetPlayer, 0.1)  -- Predict with deltaTime of 0.1
